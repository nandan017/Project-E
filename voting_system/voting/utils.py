...
def is_valid_student_qr(qr_code_data):
    ...
    if len(qr_code_data) == 7 and qr_code_data[:2].isdigit() and qr_code_data[2:4].isalnum() and qr_code_data[2:4].isupper() and qr_code_data[4:].isdigit():
        ...
        if current_year - year in range(0, 4):  # Check if the year is within the valid range
            return True
    elif len(qr_code_data) == 8 and qr_code_data[:2].isdigit() and qr_code_data[2:5].isalnum() and qr_code_data[2:5].isupper() and qr_code_data[5:].isdigit():
        year = int(qr_code_data[:2])
        if current_year - year in range(0, 4):  # Check if the year is within the valid range
            return True
        return True
    elif len(qr_code_data) == 7 and qr_code_data[:2].isdigit() and qr_code_data[2:5].isalnum() and qr_code_data[2:5].isupper() and qr_code_data[5:].isdigit():
        year = int(qr_code_data[:2])  # Extract the two-digit year
        if current_year - year in range(0, 4):  # Check if the year is within the valid range
            return True
    else:
